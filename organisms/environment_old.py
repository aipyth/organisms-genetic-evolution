...
class Simple2DContinuousEnvironment:
    ...
    def organism_result_to_coordinates(self, code: str, result: np.ndarray) -> np.ndarray:
        if code not in self.organisms_coordinates:
            raise KeyError("invalid organism code accessed")

        coordinates_bounds = self._coordinates_bounds  # (min, max)
        last_coordinates = self.organisms_coordinates[code]
        coordinates = last_coordinates + self._viscosity * result.flatten()

        # Keep the organism in the environment bounds
        coordinates = np.clip(coordinates, *coordinates_bounds)

        # Check for collisions with walls and other organisms
        direction = self._avoid_collisions(code, coordinates, last_coordinates)

        # Keep the organism in the environment bounds after collision avoidance
        coordinates = np.clip(last_coordinates + direction, *coordinates_bounds)
    ...
    def _avoid_collisions(self, code: str, coordinates: np.ndarray, last_coordinates: np.ndarray) -> np.ndarray:
