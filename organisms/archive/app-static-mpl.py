...
# use another backend to speed up things with matplotlib
matplotlib.use('agg')
...
def main():
    ...
    env = Simple2DContinuousEnvironment(width=WIDTH,
                                        ...
                                        food_size=FOOD_SIZE,
                                        food_energy=1,
                                        food_appearance_number_rate=1)
    ev = evolve.Evolve(
        encoding=encode.RealValued(),
        selection=evolve.TruncationSelection(n=4),
        crossover=evolve.SBXCrossover(n=5),  # type: ignore
    ...
    plot_threads: list[threading.Thread] = []
    with tqdm(total=ITERATIONS, desc="Simulating organisms") as pbar:
        for i in range(ITERATIONS):
            ...
            t = threading.Thread(target=plot_frame,
                                 args=(organisms_loc_df, food_loc_df, i))
            t.start()
    ...
    # wait for all plotting threads to finish execution
